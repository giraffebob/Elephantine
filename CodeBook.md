#This is the CodeBook for the small tidy data set contained in tidyData.txt

The data has a header row, plus another 180 rows and 68 columns. 
The first two columns uniquely identify the rows. All subject-activity combinations are present.
The remaining columns contain numerical data.

The data relates to motion measurements using the inbuilt accelerometer and gyroscope of a smartphone.
The experiment in which the data was collected is described in the Appendix below, with a reference to the relevant paper.
...
The readings have not been subjected to the FFT.
It's an acceleration, not a jerk.
Readings were normalised against their largest values, so lie in range [-1,1]. (Applies to all variables.) 
A mean was taken over all readings for this experiment.
Finally, a mean was taken over all experiments (rows of the bigger tidy data set) for this subject-activity combination. (Applies to all variables.) 
...
[Note - there are some inconsistencies between this description and 'features.txt'.]
